...
\experience
	...
	{Software Engineer, Site Reliability Engineering}
	{
		...
		Highlights
		\begin{projectList}
			\item{Made schema changes safer for hundreds of internal databases by rolling out push time safety checks}
			\item{Reduced crash rates, and improved behavior when a critical subsystem is unavailable}
			\item{Identified, escalated, and recovered a data loss incident (\#\#k items affected)}
			\item{On-call rotation with 12 hour shifts, 5 minute response time}
			\item{Performed regular technical interviews}
			\item{Improved the team on-boarding process, mentored teammates, hosted interns}
		\end{projectList}
	}
...
\experience
	...
	{Implemented the Smart Text Select animation introduced in Android Pie.}
